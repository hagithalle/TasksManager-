...
export default function AddTaskDialog({ open, onClose, onAdd, onEdit, goals, userId, defaultGoalId, editTask }: AddTaskDialogProps) {
  ...
  return (
    <Dialog open={open} onClose={onClose} fullWidth maxWidth="sm" PaperProps={{ sx: { borderRadius: 4 } }}>
      <DialogTitle sx={{ fontWeight: 700 }}>{isEdit ? t('task.edit', 'עריכת משימה') : t('task.new')}</DialogTitle>
          ...
          <Box>
            ...
            <Box sx={{ display: 'flex', gap: 1, mb: 0.5 }}>
              <TextField
                ...
                size="small"
                fullWidth
                placeholder={t('task.addSubtask', 'הוסף תת-משימה...')}
                value={subTaskInput}
            ...
            <Box sx={{ display: 'flex', gap: 1, mb: 1 }}>
              ...
              <TextField
                size="small"
                type="number"
                placeholder={t('task.durationLabel', 'דקות')}
              ...
              <Box key={idx} sx={{ display: 'flex', alignItems: 'center', gap: 0.5, py: 0.25, pl: 0.5 }}>
                <Box sx={{ flex: 1 }}>
                  ...
                  <Box sx={{ display: 'flex', gap: 0.5, flexWrap: 'wrap', mt: 0.25 }}>
                    ...
                    {st.durationMinutes && (
                      <Typography variant="caption" color="text.disabled">{st.durationMinutes}{t('task.minutesShort', 'ד׳')}</Typography>
                    )}
                  </Box>
                </Box>
...
export default function AddTaskDialog({ open, onClose, onAdd, goals, userId, defaultGoalId }: AddTaskDialogProps) {
  const { t } = useTranslation()
